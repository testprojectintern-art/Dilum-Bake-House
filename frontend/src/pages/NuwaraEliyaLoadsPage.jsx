...
import { Loader2, Search, Calendar, FileText, ArrowUpRight, Inbox } from 'lucide-react';
...
export default function NuwaraEliyaLoadsPage() {
    ...
    // Flatten all loaded items across all trips
    const loadedLines = [];
    deliveries.forEach(d => {
        if (d.loadedItems) {
            d.loadedItems.forEach(item => {
    ...
    // KPI Aggregates
    const totalQty = loadedLines.reduce((sum, line) => sum + line.qty, 0);
    const totalValue = loadedLines.reduce((sum, line) => sum + line.subtotal, 0);
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (row) => <span className="font-bold text-gray-700">{row.qty}</span>
        },
        {
            key: 'subtotal',
            label: 'Subtotal Value',
            render: (row) => <span className="font-bold text-slate-800">{formatPrice(row.subtotal)}</span>
        }
    ];
    ...
    return (
        <div className="space-y-6">
            <PageHeader
                title="Loaded Products (Loads)"
                description="List of all products loaded and sent to Nuwara Eliya across different consignment trips."
            />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Card className="p-4 flex items-center justify-between bg-white border-gray-100 hover:shadow-md transition">
                    <div className="space-y-1">
                        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Total Loaded Quantity</p>
                ...
                <Card className="p-4 flex items-center justify-between bg-white border-gray-100 hover:shadow-md transition">
                    <div className="space-y-1">
                        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Total Loaded Value</p>
                        <h3 className="text-xl font-bold text-indigo-650">{formatPrice(totalValue)}</h3>
                    </div>
                    <div className="w-10 h-10 rounded-xl bg-indigo-50 flex items-center justify-center">
            ...
            <Card className="p-6">
                {isLoading ? (
                    ...
                    <EmptyState
                        ...
                        description="There are no products listed as loaded matching your filters."
                    />
                ) : (
                    <Table columns={columns} data={loadedLines} />
                )}
            </Card>
        </div>
    );
